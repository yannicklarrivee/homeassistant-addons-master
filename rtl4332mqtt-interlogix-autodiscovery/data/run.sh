...
do
    PROTOCOL="$(jq --raw-output '.radios['$DEVICE'].protocol' $CONFIG_PATH)"
    FREQUENCY="$(jq --raw-output '.radios['$DEVICE'].frequency' $CONFIG_PATH)"
    GAIN="$(jq --raw-output '.radios['$DEVICE'].gain' $CONFIG_PATH)"
    OFFSET="$(jq --raw-output '.radios['$DEVICE'].frequency_offset' $CONFIG_PATH)"
