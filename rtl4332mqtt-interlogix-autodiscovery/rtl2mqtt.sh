...
python3 -u /config/rtl4332mqtt-interlogix-autodiscovery/rtl_433_mqtt_hass.py & /usr/local/bin/rtl_433 -f $FREQUENCY -R $PROTOCOL -g $GAIN -p $OFFSET -s 1000k -F "mqtt://$MQTT_HOST,user=$MQTT_USER,pass=$MQTT_PASS,events=$MQTT_TOPIC/devices[/model][/subtype][/id]/events,states=$MQTT_TOPIC/devices[/model][/subtype][/id]/states,devices=$MQTT_TOPIC/devices[/model][/subtype][/id]"
